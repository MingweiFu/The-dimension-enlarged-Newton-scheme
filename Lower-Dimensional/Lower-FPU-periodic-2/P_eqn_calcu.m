...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Output:                                                            %%%
%%% Fr_matrix_ori:  P-equations values in forms of matrix of size      %%%
%%%                 (Ar*A, n) without deleting the Q-eqns              %%%
%%% Fr_matrix_nan:  P-equations values in forms of matrix of size      %%%
%%%                 (Ar*A, n) setting nan at the Q-equations           %%%
...
function [Fr_matrix_ori, Fr_matrix_nan, Fr_vec_tot_nan, Fr_vec_red] = P_eqn_calcu(z_appro, mu_appro, mu, eps, n, torus_idx, A, V)
...
Ar_expand = 3 * Ar;             % Ar_expand = 2 * A^r
Lr_expand = 2 * Ar_expand + 1;  % Lr_expand = 2 * (3A^r) + 1

H1_partial_barz = zeros(Lr_expand, n);  
for j = 1 : n
    H1_partial_barj = zeros(Lr_expand, 1);
...
for j = 1 : n
    H1_partial_barz_next(offset + 1 : offset + Lr_expand, j) = H1_partial_barz(:, j);  % k \in [-A^{r+1}, A^{r+1}]
end
...
for j = 1 : n
    Linear_z_next(offset_1 + 1 : offset_1 + Lr, j) = Linear_z(:, j);
end
