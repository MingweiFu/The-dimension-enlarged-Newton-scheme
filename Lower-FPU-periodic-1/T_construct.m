%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% 2026/02/14  Mingwei Fu                                             %%%
%%% This function constructs the linearized operator                   %%%
%%% T_{A^{r+1}}, which is size ( (2A^{r+1}+1) * n - m ) \times         %%%
%%% ( (2A^{r+1}+1) * n - m ).                                          %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Input:                                                             %%%
%%% z_appro:   matrix of z_j (j = 1:n) coefficients                    %%%
%%% mu_appro:  approximate frequency (dim = m = 1)                     %%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Output:                                                            %%%
%%% T: linearized matrix of size ( (2A^{r+1}+1) * n - m )  ×           %%%
%%%    ( (2A^{r+1}+1) * n - m ), with resonant entries reduced         %%%
...
[~, ~, D_vec_red_tot] = Vectorization_Process(D_part_matrix, A_next, torus_idx); 
...
for j1 = 1 : n
    for j2 = 1 : n
        hess_sum = zeros(Lr_expand, 1);
        for j3 = 1 : n
            for j4 = 1 : n
                if V(j1, j2, j3, j4) == 0, continue; end
                
                hess_sum = hess_sum + 12 * V(j1, j2, j3, j4) * conv(q{j3}, q{j4});
            end
        end
...
% Expand each sub-block of the Hessian matrix to the interval [-A^{r+1}, A^{r+1}]
offset_2 = A_next - Ar_expand;
H1_partial_Hessian_next = cell(n, n);

for j1 = 1 : n
    for j2 = 1 : n
        submat_temp = zeros(L_next, 1);
        submat_temp(offset_2 + 1 : offset_2 + Lr_expand) = H1_partial_Hessian{j1, j2};
...
% Identify masks for displacements within the valid support set [-Ar_expand, Ar_expand]
mask_diff = (abs(dk_diff) <= Ar_expand); 
mask_sum  = (abs(dk_sum)  <= Ar_expand);
...
% Expand z_appro to [-A^{r+1}, A^{r+1}] and pad with zeros
offset_1 = A_next - Ar;
